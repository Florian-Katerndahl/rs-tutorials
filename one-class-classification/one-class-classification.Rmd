---
title: "One-class classification of a Sentinel-2 image with R"
author: "Fabian Fassnacht"
output: github_document
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(eval = TRUE, warning = FALSE)
knitr::opts_knit$set(root.dir = "C:/Remote_Sensing/1_Fernerkundung/One_class_classification",
                     global.device = TRUE)
```

```{r preparation, include=FALSE, eval=FALSE}
download.file(
  "https://drive.google.com/uc?export=download&id=1WY7vkxUOc3qUIBGqtVpVwYP4nkycYWJw&confirm=t",
  destfile = "occ.zip"
)
...
The datasets applied the tutorial are available here: [https://drive.google.com/file/d/1WY7vkxUOc3qUIBGqtVpVwYP4nkycYWJw/view?usp=sharing](https://drive.google.com/file/d/1WY7vkxUOc3qUIBGqtVpVwYP4nkycYWJw/view?usp=sharing)
...
Additionally, we set our working directory accordingly:

```{r, eval=FALSE}
setwd("C:/Remote_Sensing/1_Fernerkundung/One_class_classification")
```
...
```{r}
img <- rast("pixel/s2_10m_constitucion_sub2.tif")
```
...
target_tr <- vect("pixel/", "grassl") 
# load background data
backgr <- vect("pixel/", "backgr")
```

We can now have a look at the locations of the sample pixels for the grasslands by running the following command. Be aware, that this plot will only work if you have also run the `plotRGB` command above. In case you did not, run the command now.
...
```{r, include=FALSE}
# turn off graphics device for clean output
dev.off()
```
...
```{r}
tr_data <- extract(img, target_tr, fun=mean, ID=FALSE)
all_data <- extract(img, backgr, fun=mean, ID=FALSE)
```

You are already familiar with this command from the supervised classification tutorial.

We can have a quick look at the extracted values by running the following two commands, which will give us the following two console outputs:
...
values(occ_res1) <- as.integer(pred_occ1)
...
```{r out.width="100%"}
par(mfrow=c(1,2))
plot(occ_res1, mar = c(1, 1, 1, 1))
plot(occ_res2, mar = c(1, 1, 1, 1))
```

```{r, include=FALSE}
# turn off graphics device for clean output; does nothing, because plots don't look the same as Fabians
dev.off()
```
...
```{r}
# classifier 1
pos1 <- as.numeric(extract(occ_res1, target_tr, fun=mean, ID=FALSE)[, 1, drop=TRUE])
neg1 <- as.numeric(extract(occ_res1, backgr, fun=mean, ID=FALSE)[, 1, drop=TRUE])
...
# classifier 2
pos2 <- as.numeric(extract(occ_res2, target_tr, fun=mean, ID=FALSE)[, 1, drop=TRUE])
neg2 <- as.numeric(extract(occ_res2, backgr, fun=mean, ID=FALSE)[, 1, drop=TRUE])
...
Again, we set our working directory:
...
```{r}
#load image
img <- stack("object/scaled_S1.tif")
...
segm <- readOGR("object/", "small")
# load training segments of target class
segm_tr <- readOGR("object/", "small_tr")
...
```{r}
tr_data <- extract(img, segm_tr, fun=mean, ID=FALSE)
all_data <- extract(img, segm, fun=mean, ID=FALSE)
```
...
```{r, eval=FALSE}
load("object/tr_data2.RData")
load("object/all_data2.RData")
```
...
```{r, eval=FALSE}
save(tr_data, file="object/tr_data2.RData") 
save(all_data, file="object/all_data2.RData")
```
...
par(mfrow=c(2,1))
plot(class_res1, col=class_res1$occ_cl, mar = NA)
...
plot(class_res2, col=((class_res2$occ_cl)-1), mar = NA)
```
